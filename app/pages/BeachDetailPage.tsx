...
import { getAttractionsByBeachId } from '../data/beach-attractions';
import { getRestaurantsByBeachId } from '../data/beach-restaurants';
...
const BeachDetailPage: React.FC = () => {
  ...
  const attractions = getAttractionsByBeachId(beach.id);
  const restaurants = beach.restaurants
    ? getRestaurantsByBeachId(beach.id)
    : [];
      ...
      <div className='relative h-[60vh] overflow-hidden'>
        ...
        <div className='absolute bottom-0 left-0 right-0 p-8'>
          <div className='mx-auto max-w-7xl'>
            <h1 className='mb-2 text-4xl font-bold text-white'>{beach.name}</h1>
            <div className='flex items-center space-x-2 text-white/90'>
              <MapPin className='w-5 h-5' />
              <span className='text-lg'>{beach.region}</span>
            </div>
          </div>
        </div>
      </div>
